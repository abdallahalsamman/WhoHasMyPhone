...
public class FaceRecognitionAppActivity extends AppCompatActivity implements CameraBridgeViewBase.CvCameraViewListener2 {
    ...
    private void addLabel(String string) {
        String label = isNormalizedLabel(string) ? string : normalizeLabel(string);
        imagesLabels.add(label); // Normalize and Add label to list of labels
    ...
    /**
     * normalize label with the following rules:
     * 1. remove all non-alphanumeric characters
     * 2. convert to lowercase
     * 3. remove leading and trailing whitespace
     * 4. replace all whitespace with a single underscore
     * 5. truncate to 32 characters
     * 6. add a random 4 digit number to the end
     *
     * @param label
     * @return normalized label
     */
    private String normalizeLabel(String label) {
        String normalizedLabel = label.replaceAll("\\s+", "_").replaceAll("[^A-Za-z0-9_]", "").toLowerCase().trim();
        if (normalizedLabel.length() > 32)
            normalizedLabel = normalizedLabel.substring(0, 32);
        return normalizedLabel + "_" + String.format("%04d", (int) (Math.random() * 10000));
    }

    /**
     * is normalized label
     * @param label
     * @return true if label is normalized
     */
    private boolean isNormalizedLabel(String label) {
        return label.matches("[A-Za-z0-9_]{1,32}_[0-9]{4}");
    }
        ...
        mRadioButtonEigenfaces.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (!trainFaces()) {
                    ...
                    mRadioButtonEigenfaces.setChecked(false);
                    mRadioButtonFisherfaces.setChecked(true);
                }
            }
        });
        mRadioButtonFisherfaces.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if (!trainFaces()) {
                    ...
                    showToast("Still training...", Toast.LENGTH_SHORT);
                    mRadioButtonEigenfaces.setChecked(true);
                    mRadioButtonFisherfaces.setChecked(false);
                }
            }
        });
    ...
    public void onStart() {
        super.onStart();
        // Read threshold valuevices excluded in device catalog are not shos
